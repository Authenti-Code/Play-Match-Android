...
//        apiService = ApiClient().getClient().create(ApiService::class.java)
    ...
    suspend fun editSportsList(): ResultResponse {
        return try {
            val response = apiService?.editSportsList(ApiConstant.BEARER_TOKEN + " " + token)
            if (response?.isSuccessful!!) {
                val model = response.body()
                ResultResponse.Success(model)
            } else {
    ...
    suspend fun editMatch(editMatchPost: EditMatchPost): ResultResponse {
        return try {
            val response = apiService?.editMatch(ApiConstant.BEARER_TOKEN + " " + token,editMatchPost)
            if (response?.isSuccessful!!) {
                val model = response.body()
                ResultResponse.Success(model)
            } else {
    ...
    suspend fun home(): ResultResponse {
        return try {
            val response = apiService?.home(ApiConstant.BEARER_TOKEN + " " + token)
            if (response?.isSuccessful!!) {
                val model = response.body()
                ResultResponse.Success(model)
            } else {
